(function () {
  if (globalThis.__ASREdgeAishellTechCantoneseDataApiInstalled === true) {
    return;
  }
  globalThis.__ASREdgeAishellTechCantoneseDataApiInstalled = true;
  ...
  // AI 输出必须逐字符原样写入：不折叠空格、不转换繁简、不补标点。
  function normalizeCantoneseListenText(value) {
    return typeof value === "string" ? value : "";
  }

  function normalizeMarkCompareText(text) {
    return normalizeCantoneseListenText(text);
  }

  function collapseCantoneseSpaces(text) {
    return String(text || "").replace(/[\s\u3000]+/g, " ").trim();
  }

  function normalizeCantoneseTranscriptionText(text) {
    return normalizeCantoneseListenText(text);
  }
  ...
  function findMarkTextInput(documentLike) {
    const source =
    ...
    for (const row of rows) {
      ...
      if (labelFor === "text" || labelText.indexOf("文本") >= 0) {
        return row.querySelector("input.el-input__inner[type='text']");
      }
    }
    const firstRow = rows[0];
    return firstRow && typeof firstRow.querySelector === "function"
      ? firstRow.querySelector("input.el-input__inner[type='text']")
      : null;
  }

  function getCurrentInputValue() {
    return readDisplayedMarkText(document);
  }
  ...
  function readDisplayedMarkText(documentLike) {
    const input = findMarkTextInput(documentLike);
    return isInputElement(input) ? normalizeCantoneseListenText(input.value) : "";
  }

  function getTextInput() {
    const input = findMarkTextInput(document);
  ...
  function extractSavedMarkText(result) {
    ...
    if (source.mark && typeof source.mark === "object") {
      return normalizeCantoneseListenText(source.mark.text);
    }
    if (typeof source.mark === "string") {
      const parsed = safeJsonParse(source.mark);
      if (parsed && typeof parsed === "object") {
        return normalizeCantoneseListenText(parsed.text);
      }
      return normalizeCantoneseListenText(source.mark);
    }
    return normalizeCantoneseListenText(source.text);
  }
  ...
  function buildSaveShortMarkPayload(item, value, options) {
    ...
    const listenText = normalizeCantoneseListenText(
  ...
  function doesRenderedItemMatch(expected) {
    ...
    const referenceMatched =
      ...
      normalizeCantoneseTranscriptionText(currentReferenceText) ===
      normalizeCantoneseTranscriptionText(expectedReferenceText);
  ...
  function buildItemFromRecord(record, routeParams, taskDetail, options, fallbackIndex) {
    ...
    function fillPageText(value) {
      ...
      const nextText = normalizeCantoneseListenText(
        value && typeof value === "object" ? value.text : value
      );
      if (!nextText) {
        return {
      ...
      return {
        ok: true,
        message: "已原样填入当前条文本，请人工复核后决定是否保存。",
      };
    }

    function isSaveStopRequested(options) {
      return options?.signal?.aborted === true;
    }

    function createStoppedSaveResult() {
      return {
        ok: false,
        message: "本轮批量保存已停止。",
      };
  ...
  const api = {
    ...
    doesRenderedItemMatch,
    normalizeCantoneseTranscriptionText,
    normalizeCantoneseListenText,
    extractPlatformAccountName,
    extractFileNameLineText,
    extractSavedMarkText,
    extractAuthTokenFromUnknown,
    findMarkTextInput,
    readDisplayedMarkText,
  ...
  globalThis.__ASREdgeAishellTechCantoneseDataApi = api;
})();
