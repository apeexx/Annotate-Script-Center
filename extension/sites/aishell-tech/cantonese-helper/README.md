...
- 粤语助手以唯一纯数字 `data-region-label` 建立蓝色波形区段主目录，编号可稀疏且始终优先。若页面完全没有纯数字主段，严格“说话人 S1–S4:局部编号”节点可与未保存手绘节点共同按位置建目录：手绘节点必须是空 `data-region-label`、完整匹配的 `wavesurfer_` 前缀 ID，并具备完整 `px` 值的有效 CSS 几何；全部节点与按钮精确 `1…N` 对应后，按 CSS `left` 升序映射为全局 `1…N`，同一 `left` 仅按波形区域 DOM 顺序作次级排序。若最小纯数字主段为 `N>1`，其左侧恰好有 `N-1` 个严格说话人节点时也使用同一排序规则映射为前缀 `1…N-1`。不读取冒号后的局部编号，不使用标题时间，也不以 DOM 顺序跨越不同 `left` 推断；未知非空标签、临时 ID 异常或几何无效会拒绝该全位置映射。前缀映射失败只作为对应段的预检失败，不阻断安全的纯数字段。按编号精确匹配当前 `regionSelected` 按钮和页面“当前选择”，以当前“截取时长”校准区段 CSS `left / width` 对应的像素时间。区段标题仅作非阻断诊断，不参与裁剪范围计算或识别拒绝。
