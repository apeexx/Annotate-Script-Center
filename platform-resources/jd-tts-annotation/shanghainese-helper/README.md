...
- “文本:”或“文本：”所在 `.cell` 内仅有一个 textarea 时，才挂载“上海话 AI 信息”卡并锁定该文本框；标签可被 `FONT` 等页面结构包裹。“拼音:”或“拼音：”及 textarea 不唯一的 `.cell` 绝不作为候选。其中回填前步骤为“校验完整 WAV”。详细信息仅含模型、耗时、Token、费用、排队、缓存和 requestId。
...
- 只定位含“文本:”或“文本：”标签的 `.cell` 内唯一 textarea，原样设置其值并派发 `input`；标签可被 `FONT` 等页面元素包裹。
