# 平台与脚本索引

本索引只记录当前 public 版本仍保留的平台和脚本入口。处理具体平台前，先阅读对应运行时 README、平台资料 README 和后端说明。

## Alibaba LabelX

- 运行时代码：`extension/sites/alibaba-labelx/`
- 平台资料：`platform-resources/alibaba-labelx/README.md`
- 统一后端：`platform-resources/alibaba-labelx/asr-transcription/backend/README.md`、`platform-resources/alibaba-labelx/asr-judgement/backend/README.md`

## DataBaker

- 运行时代码：`extension/sites/data-baker/round-one-quality/`
- 平台资料：`platform-resources/data-baker/README.md`
- 脚本资料：`platform-resources/data-baker/round-one-quality/README.md`

## Magic Data

- 运行时代码：`extension/sites/magic-data/`
- 平台资料：`platform-resources/magic-data/README.md`
- 客家话资料：`platform-resources/magic-data/hakka-helper/README.md`
- 闽南语资料：`platform-resources/magic-data/minnan-helper/README.md`

## Aishell Tech

- 运行时代码：`extension/sites/aishell-tech/`
- 平台资料：`platform-resources/aishell-tech/README.md`
- 闽南语资料：`platform-resources/aishell-tech/minnan-helper/README.md`
- 粤语资料：`platform-resources/aishell-tech/cantonese-helper/README.md`
- 粤语运行时：`extension/sites/aishell-tech/cantonese-helper/README.md`（当前蓝色区段裁剪识别）
- 越南语资料：`platform-resources/aishell-tech/vietnamese-helper/README.md`
- 泰语资料：`platform-resources/aishell-tech/thai-helper/README.md`
- 中英短剧资料：`platform-resources/aishell-tech/cn-en-short-drama/README.md`

## Abaka AI

- 运行时代码：`extension/sites/abaka-ai/task-page/`
- 平台资料：`platform-resources/abaka-ai/README.md`
- Task21 资料：`platform-resources/abaka-ai/task21/README.md`

## Haitian uTrans

- 运行时代码：`extension/sites/haitian-utrans/audio-download-helper/`
- 平台资料：`platform-resources/haitian-utrans/README.md`
