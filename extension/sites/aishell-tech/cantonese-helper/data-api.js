(function () {
  ...
  function normalizeCantoneseSpeedValue(value) {
  ...
  function normalizeMarkCompareText(text) {
    return normalizeCantoneseTranscriptionText(text || "");
  }
  ...
  function normalizeCantoneseTranscriptionText(text) {
    const value = collapseCantoneseSpaces(
      String(text || "")
        .replace(/\.\.\.+/g, "…")
        .replace(/[，、,]/g, "，")
        .replace(/[。｡.]/g, "。")
  ...
  function getCurrentSpeedValue() {
    return normalizeCantoneseSpeedValue(readDisplayedMarkFieldValues(document).speed);
  }
  ...
  function extractSavedMarkFields(result) {
    ...
    if (source.mark && typeof source.mark === "object") {
      return {
        ...
        speed: normalizeCantoneseSpeedValue(source.mark.speed || ""),
      };
    }
    if (typeof source.mark === "string") {
      ...
      if (parsed && typeof parsed === "object") {
        return {
          ...
          speed: normalizeCantoneseSpeedValue(parsed.speed || ""),
        };
      }
      return {
    ...
    return {
      ...
      speed: normalizeCantoneseSpeedValue(source.speed || ""),
    };
  }
  ...
  function buildSaveShortMarkPayload(item, value, options) {
    ...
    const normalizedText = normalizeCantoneseTranscriptionText(payloadValue.text || "");
    const normalizedSpeed = normalizeCantoneseSpeedValue(payloadValue.speed || "");
    return {
      mark: JSON.stringify({
        text: normalizedText,
  ...
  function buildItemFromRecord(record, routeParams, taskDetail, options, fallbackIndex) {
    ...
    function fillPageFields(value) {
      ...
      const nextText = normalizeCantoneseTranscriptionText(source.text);
      const nextSpeed = normalizeCantoneseSpeedValue(source.speed);
    ...
    async function clickSaveAndWait(options) {
      ...
      const expectedSpeed = normalizeCantoneseSpeedValue(options?.expectedSpeed || "");
      ...
      while (Date.now() < deadline) {
        ...
        if (now - networkCheckAt >= 450) {
          ...
          try {
            ...
            const speedMatched =
              !expectedSpeed ||
              normalizeCantoneseSpeedValue(savedFields.speed) === expectedSpeed;
