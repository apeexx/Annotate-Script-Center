...
| 目标 | 稳定规则 | 使用方式 |
| --- | --- | --- |
| 文本标签 | `div.cell > span:first-child` 且文本精确为 `文本:` | 作为唯一挂载起点 |
...
- 运行时仍用 `MutationObserver` 复核挂载点；若容器或文本 textarea 未来被替换，会移除旧按钮并重新挂载。
- 接收到新的语句身份时会取消进行中的请求并使旧快照失效，避免旧结果写到新条目。

## 可挂载点建议

- 仅在“文本:”标签的直接相邻容器后插入按钮。
