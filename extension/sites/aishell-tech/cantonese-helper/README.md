...
- 粤语助手只识别当前带唯一纯数字 `data-region-label` 的蓝色波形区段；编号可稀疏。按编号精确匹配当前 `regionSelected` 按钮和页面“当前选择”，使用当前“截取时长”校准像素时间；区段标题只用于校验，不用于推断裁剪范围。说话人 S1–S4 覆盖层不参与音频裁剪、结果绑定或批量目录。
- 浏览器对当前原始音频只获取并解码一次，再离线裁剪对应的 `16kHz` 单声道 WAV Data URL。裁剪失败、实际区段缺少对应编号按钮、编号重复或当前选择未同步时直接拒绝，不会回退发送整条音频。
...
- 用户启动“全部”批量后，助手处理当前音频实际存在的全部带编号蓝色区段；“未完成”批量按编号切换，并以“文本”输入框为空判定。说话人覆盖层只保留为页面元数据，不进入批量任务。
