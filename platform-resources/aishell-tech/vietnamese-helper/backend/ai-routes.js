...
const AI_BASE_PATH = "/api/aishell-tech/vietnamese-helper/ai/recommend";
...
function buildCacheKeyParts(request) {
  return {
    ...
    singlePrompt: request.aiStages?.recognize?.prompt || request.singlePrompt,
  };
}
  ...
  function registerAiRoutes(router) {
    ...
    router.get(AI_LOG_SUMMARY_PATH, function ({ response, query }) {
      deps.sendJson(
        ...
        200,
        buildAiCallLogSummaryPayload({
          service: "aishell-tech-vietnamese-helper-ai-recommend",
          scriptId: SCRIPT_ID,
