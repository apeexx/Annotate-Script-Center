...
const stageLogSupport = createStageLogSupport({
  stages: [
    { key: "convert", label: "转换", modelKeys: ["convertModel", "candidateModel"] },
    { key: "listen", label: "听音", modelKeys: ["listenModel"] },
    { key: "compare", label: "比较", modelKeys: ["compareModel"] },
  ],
});
...
const aiCallLogger = createAiCallLogger({
  ...
  buildExtendedRow(context) {
    ...
    return {
      ...
      convertModel: normalizeText(models.convertModel || request.convertModel || models.candidateModel || request.candidateModel),
      listenModel: normalizeText(models.listenModel || request.listenModel),
      compareModelFamily: normalizeText(models.compareModelFamily || request.compareFamily),
...
function getAishellCantoneseAiCallLogFilePath(logDir, createdAt) {
  return path.join(normalizeText(logDir) || DEFAULT_LOG_DIR, "ai-calls-" + formatDatePart(createdAt) + ".csv");
}

function buildAishellCantoneseAiCallLogRow(input) {
  return aiCallLogger.buildRow(input);
}

function appendAishellCantoneseAiCallLog(input) {
...
module.exports = {
  ...
  DEFAULT_LOG_DIR,
  aiCallLogger,
  appendAishellCantoneseAiCallLog,
  appendAishellCantoneseAiCallLogSafe,
  buildAishellCantoneseAiCallLogRow,
  getAishellCantoneseAiCallLogFilePath,
  getLogDir,
};
