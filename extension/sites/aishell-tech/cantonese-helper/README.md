...
- 粤语助手以唯一纯数字 `data-region-label` 建立蓝色波形区段主目录，编号可稀疏且始终优先。若最小纯数字主段为 `N>1`，仅当其左侧恰好有 `N-1` 个严格格式为“说话人 S1–S4:局部编号”的节点、每个 CSS 几何有效且 `left` 不并列时，才按 CSS `left` 升序映射为全局 `1…N-1`；不读取冒号后的局部编号，不使用 DOM 顺序或标题推断。前缀映射失败只作为对应段的预检失败，不阻断安全的纯数字段。按编号精确匹配当前 `regionSelected` 按钮和页面“当前选择”，以当前“截取时长”校准区段 CSS `left / width` 对应的像素时间。区段标题仅作非阻断诊断，不参与裁剪范围计算或识别拒绝。
