...
| 蓝色波形区段 | `wave > region.wavesurfer-region`（必须按 DOM 顺序编号） |
...
- 粤语助手将波形 `region` 的 DOM 顺序与区段按钮一一对应；标题只用作整秒范围校验，实际裁剪范围由区段 `left / width` 与当前“截取时长”动态计算。按钮数、当前选择、时长或标题校验不一致时必须拒绝操作。
- 当前音频可以存在说话人覆盖区段；覆盖区段同样是独立 DOM `region`，必须保留在顺序目录中。
