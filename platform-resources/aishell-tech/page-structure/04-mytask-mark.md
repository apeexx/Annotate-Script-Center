...
| 可识别蓝色波形区段 | `wave > region.wavesurfer-region[data-region-label]`；唯一纯数字标签为主段，编号可稀疏且优先；若最小纯数字标签为 `N>1`，其左侧恰好 `N-1` 个有效说话人节点按 CSS `left` 升序映射为 `1…N-1`；区域 title 仅作非阻断诊断 |
| 说话人覆盖层 | 同为 `wave > region.wavesurfer-region`，标签如“说话人S1:1”；冒号后为说话人局部序号，不可作全局段号。仅严格 S1–S4 节点可参与前缀位置映射；数量、几何或 left 并列不满足时对应前缀段预检失败，不得猜测 |
...
- 粤语助手以唯一纯数字 `data-region-label` 建立可识别区段主目录，允许稀疏编号，并按编号与区段按钮、页面“当前选择”精确对应。若最小纯数字主段为 `N>1`，仅在左侧恰好有 `N-1` 个严格 S1–S4 说话人节点、几何有效且 `left` 不并列时，才按 CSS `left` 升序映射为 `1…N-1`；标签冒号后的局部序号、DOM 顺序和标题均不可用于推断。实际裁剪范围由区段 CSS `left / width` 与当前“截取时长”动态计算。区域 title 仅作非阻断诊断，不参与裁剪范围或识别拒绝。裁剪终点会限制到音频实际时长；限制后没有有效时间范围时，以及编号重复、对应按钮缺失、当前选择或时长不同步、CSS 几何无效时必须拒绝操作。
