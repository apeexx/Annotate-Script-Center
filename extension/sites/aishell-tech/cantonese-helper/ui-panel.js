(function () {
  if (globalThis.__ASREdgeAishellTechCantoneseUiPanelInstalled === true) {
    return;
  }
  globalThis.__ASREdgeAishellTechCantoneseUiPanelInstalled = true;
  ...
  function hasRecommendedText(result) {
    return Boolean(normalizeCompareText(result?.listenText));
  }

  function hasFillableRecommendation(result) {
    return hasRecommendedText(result);
  }
  ...
  function isResultBoundToCurrentItemKey(result, currentItemKey) {
    const resultItemKey = String(result?.itemKey || "");
    const selectedItemKey = String(currentItemKey || "");
    return !resultItemKey || !selectedItemKey || resultItemKey === selectedItemKey;
  }

  function isSameAsCurrentText(result) {
    const source = result && typeof result === "object" ? result : {};
    const listenText = normalizeCompareText(source.listenText);
    const currentText = normalizeCompareText(
      source.currentDisplayText || source.currentText || source.referenceText
    );
    return Boolean(listenText) && Boolean(currentText) && listenText === currentText;
  }
  ...
  function isSameAsDisplayedCurrent(result) {
    ...
    return isSameAsCurrentText(source);
  }
  ...
  function createRuntime(options) {
    ...
    function copyCurrentRecommendedText() {
      ...
      return copyText(result.listenText || "").then(function () {
    ...
    function fillCurrentRecommendedText() {
      ...
      if (typeof deps.fillPageText !== "function") {
        return Promise.reject(new Error("当前运行时没有填入能力。"));
      }
      const selectedItemPromise =
        typeof deps.getCurrentItem === "function" ? deps.getCurrentItem() : Promise.resolve(null);
      return Promise.resolve(selectedItemPromise).then(function (selectedItem) {
        if (typeof deps.getCurrentItem === "function" && !isResultBoundToCurrentItem(result, selectedItem)) {
          updateCurrentItemKey(selectedItem?.key || "");
          const message = "当前已切换到其他条目，上一条识别结果不能填入，请重新识别。";
          setStatus(message, "warning");
          return { ok: false, message: message };
        }
        setStatus("正在原样填入当前条...", "info");
        return Promise.resolve(deps.fillPageText(result.listenText || "")).then(function (fillResult) {
          setStatus(
            fillResult?.message || "已原样填入当前条，未自动保存。",
    ...
    function ensureRoot() {
      ...
      resultsTitle.textContent = "希尔贝壳粤语识别";
      ...
      fillRecommendBtn = createButton("原样填入", {
        "data-primary": "true",
      });
    ...
    function renderResult(result) {
      ...
      const listenText = String(source.listenText || "");
      const referenceText = String(source.referenceText || "");
      const sameAsDisplayedCurrent = isSameAsDisplayedCurrent(source);
      const resultMatchesCurrentItem = isResultBoundToCurrentItemKey(source, currentItemKey);
      if (recommendTextDisplay) {
        recommendTextDisplay.textContent = listenText || "暂无识别结果";
        recommendTextDisplay.style.backgroundColor = "#f0fdf4";
        recommendTextDisplay.style.borderColor = "#bbf7d0";
        recommendTextDisplay.style.color = "#166534";
        recommendTextDisplay.style.fontStyle = "normal";
      ...
      if (fillRecommendBtn) {
        fillRecommendBtn.disabled = !canFillCurrentResult(source) || !resultMatchesCurrentItem;
      }
      if (resultHintNode) {
        if (!resultMatchesCurrentItem) {
          resultHintNode.textContent = "当前已切换到其他条目，上一条识别结果不能填入，请重新识别。";
          resultHintNode.style.color = "#b45309";
          resultHintNode.style.display = "";
        } else {
          resultHintNode.textContent = "当前与页面一致，仍可重新填入保存。";
          resultHintNode.style.color = "#047857";
      ...
      return {
        canApply: canFillCurrentResult(source) && resultMatchesCurrentItem,
  ...
  globalThis.__ASREdgeAishellTechCantoneseUiPanel = api;
