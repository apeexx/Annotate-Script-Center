...
- 区段从 `wave > region.wavesurfer-region` 中按编号建立目录：唯一纯数字 `data-region-label` 为主段，编号可以稀疏且始终优先。若最小纯数字主段为 `N>1`，仅在其左侧恰好有 `N-1` 个严格“说话人 S1–S4:局部编号”节点、CSS 几何有效且 `left` 不并列时，才按 CSS `left` 升序映射为全局 `1…N-1`；冒号后的局部编号、DOM 顺序和标题时间均不参与映射。前缀映射失败仅标记对应段预检失败，不影响安全纯数字段。目录按页面区段按钮与“当前选择”精确匹配；使用当前“截取时长”校准区段 CSS `left / width` 的像素时间。区域标题仅作非阻断诊断，不参与裁剪范围计算或识别拒绝。编号重复、按钮映射、当前选择、时长、CSS 几何或裁剪后的音频实际范围无效时拒绝识别，绝不回退整段音频。
