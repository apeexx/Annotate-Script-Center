...
- 粤语助手只识别当前蓝色波形区段：以 `wave > region.wavesurfer-region` 的 DOM 顺序和当前 `regionSelected` 按钮一一映射，使用当前“截取时长”校准像素时间；区段标题只用于校验，不用于推断裁剪范围。
...
- 用户启动“全部”批量后，助手处理当前音频全部蓝色区段；“未完成”批量逐段切换，并以“文本”输入框为空判定。说话人覆盖区段也按 DOM 顺序参与。
- 批量 AI 可乱序返回，但结果先缓冲，页面仍按区段 DOM 顺序串行“切换区段 → 校验选择/时长 → 原样填入 → 点击真实保存 → 等待成功提示”。空 `listenText` 只标记人工复核，不填入、不保存。
