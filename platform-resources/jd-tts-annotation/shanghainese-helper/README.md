...
- “文本:”或“文本：”所在 `.cell` 内仅有一个 textarea 时，才挂载“上海话 AI 信息”卡并锁定该文本框；另兼容已采样的共享结构：同一 `FONT` 内依次只有“文本:”/“文本：”和“拼音:”/“拼音：”标签，且 `.cell` 直属的两个 Element UI 输入容器中第一个容器唯一含有 textarea。共享结构仅查询、锁定和写入第一个容器的 textarea，绝不查询、读取、监听、写入或触发第二个（拼音）textarea；其他不唯一或结构不完整的 `.cell` 绝不作为候选。其中回填前步骤为“校验完整 WAV”。详细信息仅含模型、耗时、Token、费用、排队、缓存和 requestId。
...
3. 任意点选波形画段后点击“上海话识别”，确认助手仍只处理当前完整 WAV，信息卡经过健康检查、创建任务、等待结果和“校验完整 WAV”，并且只回填精确“文本:”或“文本：”textarea。
...
- 只定位含“文本:”或“文本：”标签的 `.cell` 内唯一 textarea，或已采样的“文本/拼音”共享结构中第一个直属输入容器的唯一 textarea；原样设置其值并派发 `input`，标签可被 `FONT` 等页面元素包裹。
