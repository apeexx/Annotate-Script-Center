...
- 粤语助手固定为单阶段 Omni 原始听写：唯一纯数字编号的蓝色波形区段（可为稀疏编号）始终优先；无纯数字主段时，严格说话人 S1–S4 节点可与空标签、完整匹配的 `wavesurfer_` ID、完整 `px` 几何的未保存手绘节点一起，在精确 `1…N` 按钮映射下按 CSS `left` 映射为全局段，完全相同的 `left` 仅以波形区域 DOM 顺序作次级排序；未知标签或临时节点标识异常拒绝映射，最小纯数字主段前的说话人前缀仍使用既有规则。浏览器只裁剪当前安全区段为 `16kHz` 单声道 WAV Data URL 后传入模型，绝不回退整段 OSS 音频；映射失败段不发起 AI 且不阻断安全纯数字段。唯一业务结果为原样 `listenText`，不接词表、转换、比较、Fun-ASR 或语速字段。
...
- 粤语助手当前 AI 配置固定为单阶段 Omni；结果区、复制与填入均原样使用 `listenText`。单条只填入，由用户决定是否保存；批量时当前音频所有实际存在的带编号蓝色区段按编号顺序保存，并按编号匹配对应按钮，不自动修改说话人；空结果只人工复核，保存前复核当前区段并点击页面真实保存按钮。
