(function () {
  ...
  function createRuntime(options) {
    ...
    function resolveAiOmni() {
      const source = config.aiOmni && typeof config.aiOmni === "object"
        ? config.aiOmni
        : config.aiOptions && typeof config.aiOptions === "object"
          ? config.aiOptions
          : {};
      return {
        model: normalizeText(source.model || config.singleModel || config.listenModel),
        prompt: normalizeStagePrompt(source.prompt || source.singlePrompt || source.listenPrompt),
        params: normalizeStageParams(source.params || source),
      };
    }
    ...
    function createRequestBody(item) {
      ...
      requestBody.aiOmni = {
        model: aiOmni.model,
        prompt: aiOmni.prompt,
        params: aiOmni.params,
      };
      return requestBody;
    }
